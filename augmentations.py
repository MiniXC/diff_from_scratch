...
class TimeoutQueue:
    def __init__(self, timeout_duration, timeout_callback):
        self.timeout_duration = timeout_duration
        self.timeout_callback = timeout_callback
        self.thread = threading.Thread(target=self.run_timeout)
        self.queue = queue.Queue()

    def run_timeout(self):
        try:
            self.queue.get(timeout=self.timeout_duration)
        except queue.Empty:
            self.timeout_callback()
...
def wave_augmentation_func(wave, return_name=False):
    ...
    with warnings.catch_warnings():
        ...
        with TimeoutQueue(1, timeout_message): # we use a timeout to prevent hangs
            # polarity inversion
